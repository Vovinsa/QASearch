...
def search():
    ...
    Note:
    - The 'embedder' service is expected to be running and accessible at 'http://embedder:8501'.
    ...
    - The 'ranker' service at 'http://ranker:8001' is used to rank the documents based on the query embedding.
    ...
    query_embedding = requests.post(
        'http://embedder:8501/v1/models/use:predict', json={'instances': [query]}
    ).json()['predictions'][0]
    ...
    documents_embeddings = requests.post(
        'http://embedder:8501/v1/models/use:predict', json={'instances': documents}
    ).json()['predictions']

    ranked_documents = requests.post(
        'http://ranker:8001/rank',
